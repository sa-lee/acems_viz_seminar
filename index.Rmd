...
We can use ggplot2 to create chloropleth maps.
...
```{r reldata, eval = FALSE, echo = TRUE}
tb_rel <- tb %>% 
...
Let's look at relative difference from 2012 - 2011
...
```{r chloro-out, ref.label="chlormap", echo=FALSE, cache=TRUE, fig.width=12, fig.height=6}
```
...
```{r viridis, echo = TRUE, eval = FALSE}
library(viridis)
...
```{r viridis-out, ref.label="viridis", echo=FALSE, cache=TRUE, fig.height=6, fig.width=6}
```
