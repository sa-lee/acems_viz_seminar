...
.largeish[
https://sa-lee.github.io/acems_viz_seminar/

https://github.com/sa-lee/acems_viz_seminar
]
...
```{r plotPolar, eval=FALSE, echo=TRUE}
ggplot(
  tb_au,
  aes(x = year,
      y = count,
...
```{r outputPolar, ref.label="plotPolar", echo=FALSE, cache=TRUE, fig.height = 6, fig.width=10}
```
...
```{r plotPie, eval=FALSE, echo=TRUE}
ggplot(tb_au,
       aes(
         x = 1,
         y = count,
...
```{r outputPie, ref.label="plotPie", echo=FALSE, cache=TRUE, fig.height = 6, fig.width=10}
```
...
We can use ggplot2 to create chloropleth maps. This requires
use having some information about geographical regions we
are going to draw. `ggplot2` has some basic mapping functionality
with the `map_data()` function.
...
For each country we can compute the relative difference between
TB cases between year for each country. 
...
We will look at relative difference form 2012 - 2011
...
```{r}
library(viridis)
map + scale_fill_viridis(option = "magma")
```
...
tb_map %>% 
  ...
  scale_fill_viridis(option = "magma") +
  theme_map() +
  transition_time(year, range = c(2007, 2012))
